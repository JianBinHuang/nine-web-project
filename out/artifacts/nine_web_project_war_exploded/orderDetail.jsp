...
<head>
	...
	<script type="text/javascript">
		...
		function payZhifubao(orderId){
			location.href="pay?method=aliPay&oid="+orderId;
		}
	</script>
</head>
		...
		<table cellpadding="0" cellspacing="0" align="center" width="100%" class="table table-striped table-bordered table-hover">
			...
			<tr>
				<td align="right" colspan="4" style="margin-right: 40px;">
					<a href="${pageContext.request.contextPath }/order?method=showList&uid=${loginUser.uid}" class="btn btn-danger btn-sm">返回订单列表</a>
					&nbsp;&nbsp;
...
<%--						<button type="button" onclick="pay('${od.order.id}','${od.order.money}')" class="btn btn-warning btn-sm">易付宝支付</button>--%>
						&nbsp;&nbsp;
						<button type="button" onclick="payZhifubao('${orders.oid}')" class="btn btn-success btn-sm">支付宝支付</button>
					</c:if>
				</td>
			</tr>
		</table>
