            ...
            <c:forEach items="${pageBean.list}" var="g" varStatus="i">
                <div class="col-sm-3">
                    <div class="thumbnail">
                        ...
                        <div class="caption">
                            <h4>商品名称<a href="${pageContext.request.contextPath}/product?method=getProductById&pid=${g.pid}">${g.pname}</a></h4>
                            <p>热销指数
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination">
                    <li class="${pageBean.currentPage==1?'disabled':''}">
                        <%--					${param.tid} request.getParamter("tid")--%>
                        <a href="product?method=queryProductByTypeId&tid=${param.tid}&currentPage=${pageBean.currentPage-1}" aria-label="Previous">
                    ...
                    <c:forEach begin="1" end="${pageBean.totalPage}" step="1" var="index">
                        <c:if test="${pageBean.currentPage == index}">
                            <li class="active">
                                <a href="product?method=queryProductByTypeId&tid=${param.tid}&currentPage=${index}">${index}</a>
                            </li>
                        </c:if>
                        <c:if test="${pageBean.currentPage != index}">
                            <li>
                    ...
                    <li class="${pageBean.currentPage==pageBean.totalPage?'disabled':''}">
                        <a href="product?method=queryProductByTypeId&tid=${param.tid}&currentPage=${pageBean.currentPage+1}" aria-label="Next">
